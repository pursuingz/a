...
with open('input.fasta', 'r') as f:
    ...
    for line in f:
        ...
        #sequence += '\n'
# 压缩序列
header_info , lower_case_info , non_ACGT_info , Substring_info , compressed_seq = compress(sequence)

# 将压缩结果写入新文件中
with open('./x/Heads_file.txt', 'w') as f:
    f.write(header_info)  # 写入头部信息
with open('./x/Lowercase_file.txt', 'w') as f:
    f.write(lower_case_info)  # 写入小写字母列表
with open('./x/Non_ATCG_file.txt', 'w') as f:
    f.write(non_ACGT_info)  # 写入非ATCG字母信息
with open('./x/Substring_file.txt', 'w') as f:
    f.write(Substring_info)  # 写入分离的重复子段
with open('./x/main_file.txt', 'w') as f:
    f.write(compressed_seq)  # 写入二进制编码后的序列
with py7zr.SevenZipFile('target.7z', 'w') as archive:
    archive.writeall('./x', 'base')
